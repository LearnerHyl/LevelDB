# LevelDB-Compaction理论

要注意Compaction与Multi-Version是息息相关的，因此最好将两部分结合在一起看。这部分多是理论，具体的请见Multi-Version部分，那部分结合了源代码，分析起来更有条理一些。

## Compaction的作用

正如Rebalance与Spill之于B+Tree，Compaction操作是LSM-Tree的核心。LevelDB中LSM-Tree的Compaction操作分为两类，分别是Minor Compaction与Major Compaction：
...
在LevelDB中，Major Compaction还可以按照触发条件分为三类，在上一节的Multi-Version中已经具体介绍过size compaction和seek compaction的源码：
...
> Minor Compaction就是将Immutable Memtable转化为第0层的SSTable的过程。

一次minor compaction的产出是一个0层的sstable文件，其中包含了所有的内存数据。但是第0层的SSTable文件中是可能存在数据overlap的；而从Level1开始再往下，这些层级中的SSTable文件中的键不仅仅是有序的，而且每个层级中的所有文件也不会出现键的overlap现象。

### 提高读写效率

正如前面的文章提到，leveldb是一个写效率十分高的存储引擎，存储的过程非常简单，只需要一次顺序的文件写和一个时间复杂度为O(log n)的内存操作即可。

> LevelDB的MemTable底层是一个SkipList结构，查找跳表的时间复杂度为O(log n)。
...
<img src="./assets/image-20240604220250733.png" alt="image-20240604220250733" style="zoom: 80%;" />

每次minor compaction结束后，都会生成一个新的sstable文件，也意味着**Leveldb的版本状态发生了变化，会进行一个版本的更替**。与版本控制相关的内容，将在下一篇文章中说明。
...
<img src="./assets/image-20240604220709630.png" alt="image-20240604220709630" style="zoom:67%;" />

0层中浅蓝色的三个sstable文件，加上1层中的绿色的sstable文件，四个文件进行了合并，输出成两个按序组织的新的1层sstable文件进行替换。<u>由上图可知，除了Level0层的文件中的key会有overlap之外，从level1开始往下的层级，每个层级内部的SSTable文件中的key都是不重合的，但是注意不同Level存储的SSTable文件是有key重合的。</u>

#### 触发Major Compaction的条件
...
major compaction分为size compaction和seek compaction。
...
#### 采样探测-错峰合并（seek compaction机制)

在每个sstable文件的元数据中，还有一个额外的字段`allow_seeks_`，默认为文件的大小除以16KB。

leveldb在正常的数据访问时，会顺带进行采样探测。正常的数据访问包括（1）用户直接调用Get接口（2）用户使用迭代器进行访问。

采样的规则：

记录本次访问的第一个sstable文件。若在该文件中访问命中，则不做任何处理；若在该文件中访问不命中，则对 该文件的`allow_seeks_`标志做减一操作。

直到某一个文件的`allow_seeks_`标志减少到0时，触发对该文件的错峰合并。
...
对于*错峰合并*，起始输入文件则为该查询次数过多且每次查询都不命中的文件，上面提到过，当某个文件的allow_seeks_字段减少到0时，就会触发错峰合并。
...
![image-20240605095756086](./assets/image-20240605095756086.png)
...
- 对于0层文件，该层的分数为文件总数／4；LevelDB规定第0层的文件数量默认不能超过4个，这是为了查询性能考虑。
